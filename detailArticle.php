...
$artikel = query("SELECT ag.id_agent, ag.nama_agent, ag.foto AS agent_foto, ar.id_artikel, ar.judul, ar.kategori, ar.artikel, ar.header1, ar.artikel1, ar.header2, ar.artikel2, ar.tgl_membuat, ar.foto AS artikel_foto FROM agent AS ag CROSS JOIN artikel AS ar ON ar.id_agent = ag.id_agent WHERE ar.id_artikel='$id_artikel'")[0];
                    ...
                    <?php foreach ($related_artikel as $value): ?>
                        <div class="card flex flex-col justify-center bg-white p-3 gap-5 ">
                            ...
                            <div class="flex items-center ps-3 gap-3">
                                ...
                                <p><?= $value['nama_agent'] ?></p>
                            </div>
                            <p class="ms-3 text-sm text-slate-700"><?php

                                                                    $artikel_content = $value['artikel'];
                                                                    $maxLength = 100;
                                                                    if (strlen($artikel_content) > $maxLength) {
                                                                        $artikel_content = substr($artikel_content, 0, $maxLength) . '...';
                                                                    }
